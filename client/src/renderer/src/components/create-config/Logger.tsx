...
import { LuSave, LuSettings, LuUpload } from "react-icons/lu";
...
const Logger = forwardRef<LoggerHandle, LoggerProps>(function Logger({ logs, loading = false, onStatsUpdate, onDeleteLog, onIndicesChange, onKillOffsetChange }, ref) {
	...
	const configModalProps = useMemo(
		() =>
			config && {
				config,
				options: {
		...
		[config, possibleKillOffsets, possibleNameOffsets, nameIndicies, playerOneIndex, playerTwoIndex, guildIndex, killIndex, applyConfigSelection],
	);

	useEffect(() => {
		if (configModalProps) ModalManager.update(ConfigModal, configModalProps);
	}, [configModalProps]);
		...
		<div className="flex flex-col h-full w-full relative">
			<div className="flex items-center justify-between mb-1.5 gap-4">
				<div className="flex items-center gap-2 shrink-0">
					<Checkbox checked={autoScroll} onChange={(e) => setAutoScrollAndPersist(e.target.checked)} />
					<span className="text-sm text-gray-400 whitespace-nowrap">{t("logger.autoScroll")}</span>
				</div>

				<div className="hidden lg:block flex-1 min-w-0 text-center text-gray-400 text-xs truncate">
					{t("logger.formatHint")} <span className="font-semibold text-gray-300">YourGuild-FamilyName</span> {t("logger.killed")}/{t("logger.diedTo")} <span className="font-semibold text-gray-300">Enemy-FamilyName</span> {t("logger.from")}{" "}
					<span className="font-semibold text-gray-300">Guild</span>
				</div>

				<button
					onClick={() => configModalProps && ModalManager.open(ConfigModal, configModalProps)}
			...
			<div className="flex-1 flex flex-col overflow-hidden rounded-md border border-white/10 bg-black/20 mb-3">
				{logs.length > 0 && (
					<div className="flex gap-2 items-center px-2 h-8 border-b border-white/10 shrink-0">
						...
						<span className="section-label w-full max-w-32">{t("logger.table.playerOne")}</span>
						<span className="section-label w-20 text-center">{t("logger.table.result")}</span>
						<span className="section-label w-full max-w-32">{t("logger.table.playerTwo")}</span>
						<span className="text-xs invisible">{t("logger.from")}</span>
						<span className="section-label w-full max-w-32">{t("logger.table.guild")}</span>
			...
			<div className="flex gap-2 justify-center">
				<Button className="w-full" onClick={saveLogs} disabled={disabled} size="md" color="primary">
					<Icon icon={LuSave} size="sm" className="mr-2" />
					{t("logger.saveLogs")}
				</Button>
				<Button className="w-full" onClick={handleUploadToNodewar} disabled={disabled} size="md" color="outline">
